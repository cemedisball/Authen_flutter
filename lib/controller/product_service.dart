  ...
  try {
    ...
    final response = await http.post(
      ...
      headers: <String, String>{
        ...
        'Authorization': 'Bearer ${userProvider.accessToken}', // เพิ่ม Authorization Header
      },
...
Future<bool> updateProduct(BuildContext context, ProductModel product) async {
  try {
    ...
    final response = await http.put(
      Uri.parse('$apiURL/api/product/${product.id}'), // ใช้ product.id สำหรับระบุผลิตภัณฑ์
      headers: <String, String>{
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ${userProvider.accessToken}', // เพิ่ม Authorization Header
      },
    ...
    if (response.statusCode == 200) {
      return true; // อัปเดตผลิตภัณฑ์สำเร็จ
    } else {
      print('Failed to update product: ${response.body}');
      return false; // อัปเดตผลิตภัณฑ์ไม่สำเร็จ
    }
  } catch (e) {
